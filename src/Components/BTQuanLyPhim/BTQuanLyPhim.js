...
export default class BTQuanLyPhim extends Component {
    ...
    renderFilm = () => {
        return this.dataFilm.map((item, index) => {
            return <div className="col-4" key={index}>
                <div className="card text-white bg-primary mt-2">
                    {/* //chỉnh css cho các hình = nhau */}
                    <img className="card-img-top" src={item.hinhAnh} alt={item.tenPhim} style={{width:'100%',height:'300px'}}/>
                    <div className="card-body">
                        {/* //chỉnh css cho các tiêu đề cùng size */}
                        <h4 className="card-title" style={{height:50}}>{item.tenPhim}</h4>
                        <p className="card-text" style={{height:100}}>
                            {/* // nếu phim nhiều hơn 150 ký tự thì ...,  */}
                            {item.moTa.length >150 ? item.moTa.substr(0,150)+'...' :item.moTa}
                            </p>
                    </div>
                </div>
